...
import { FaBell, FaChevronDown, FaBars, FaChevronLeft, FaChevronRight } from "react-icons/fa";
import { Link, useNavigate } from 'react-router-dom';
import AppContext from '../AppContext';
import { useTranslation } from 'react-i18next';
import LanguageSelector from './LanguageSelector'; // Import the LanguageSelector component

const Navbar = ({ toggleSidebar, isSidebarExpanded, isMenuExpanded }) => {
    ...
    return (
        <nav className="bg-white shadow-sm">
            <div className="flex">
                <div className="w-[15%] bg-[#F1F2F7] hidden items-center justify-center md:flex">
                    <Link to={'/'} className="text-primaryColor  uppercase font-bold text-xl py-4">{t('inclusion')}</Link>
                </div>
                        ...
                        <div className="flex justify-between h-16">
                            <div className="hidden items-center sm:flex">
                                ...
                                >
                                    ...
                                    {isMenuExpanded ? <FaChevronLeft className="h-6 w-6 outline-none" /> : <FaChevronRight className="h-6 w-6" />}
                                </button>
                            </div>
